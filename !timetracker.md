...
<body>
    ...
    <script>
        ...
        function logActivity() {
            const activity = document.getElementById('activity').value;
            const hours = parseInt(document.getElementById('hours').value);
            // Save the hours in localStorage
            localStorage.setItem('loggedHours', hours);
            // Retrieve the logged hours from localStorage
            const loggedHours = parseInt(localStorage.getItem('loggedHours')) || 0;
            // Update the pie chart
            updatePieChart(loggedHours);
        }
        function updatePieChart(hours) {
    const pieChartCanvas = document.getElementById('pieChart');
    const ctx = pieChartCanvas.getContext('2d');
    const labels = ['Logged Hours', 'Remaining Hours'];
    const colors = ['#36a2eb', '#ff6384'];
    ...
    // Check if window.pieChart exists and is an object with a destroy function
    if (window.pieChart && typeof window.pieChart === 'object' && typeof window.pieChart.destroy === 'function') {
        window.pieChart.destroy(); // Destroy existing chart if it exists
    }
    window.pieChart = new Chart(ctx, {
        type: 'pie',
